  ...
  update(state) {
    ...
    this.rootElement.innerHTML += `
        ...
        <span style="display: ${this.isPlayersTurn ? 'inline' : 'none'}">
          ...
          <button id="endTurn" style="font-weight: ${state.G.movesLeft == 0 ? 'bold' : 'normal'}" ${state.G.drawsLeft == 0 || state.G.movesLeft == 0 || state.G.deck.length == 0 ? '' : 'disabled'}>Passar a vez</button>
        </span>
        <span style="display: ${!this.isPlayersTurn ? 'inline' : 'none'}">
